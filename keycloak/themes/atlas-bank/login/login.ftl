...
<#-- AtlasBank override: displayInfo no longer depends on realm.registrationAllowed
     (see the "info" section below for why). -->
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('username','password') displayInfo=realm.password; section>
    ...
    <#elseif section = "info" >
        <#-- AtlasBank override: always show a registration link, even though Keycloak's
             own self-registration (realm.registrationAllowed) is off — it points at the
             app's real /register flow instead of Keycloak's native one, which would
             create a login with no matching Customer record. -->
        <#if realm.password>
